...
const getActions = (actions: Action[]) => {
    ...
    const actionComponents = (comp.length > 0 ? comp : [actions]).map((actions, i) => <List className={"toolbar"} key={i}>
    ...
    return <>
        {actionComponents.map((components, index) => <React.Fragment key={index}>{components} {index + 1 < actionComponents.length}</React.Fragment>)}
    </>
