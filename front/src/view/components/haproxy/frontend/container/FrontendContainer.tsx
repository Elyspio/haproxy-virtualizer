...
function FrontendContainer(props: ReduxTypes) {
    ...
    const frontends = props.frontends ? Object.entries(props.frontends).map(([key, value]) => ({
        name: key,
        data: value
    })) : []
    ...
    return (
        <Accordion defaultExpanded={true}>
            <AccordionSummary
                ...
                id="panel1a-header"
            >
                <Typography className={"title"}>Frontends <Button className={"create-btn"} color={"primary"}
                                                                  onClick={props.createFrontend}>Add <Add/></Button></Typography>
            </AccordionSummary>
